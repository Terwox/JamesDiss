...
p_load(readr)
# Load the data
df <- read_csv("Dissertation - Pilot_May 1, 2025_16.58.csv")
...
p_load(readr)
# Load the data
df <- read_csv("Dissertation+-+Pilot_August+26,+2025_15.07.csv")
```
...
print(summary(anova_model), digits = 3)
...
```{r}
# Summary of the ANOVA
print(summary.lm(anova_model), digits = 3)
```
...
es_omega  <- omega_squared(anova_model)
es_eta_p  <- eta_squared(anova_model, partial = TRUE)
es_eta_g  <- eta_squared(anova_model, generalized = TRUE)
...
```{r}
# Add SD alongside means
stats <- aggregate(Total_Score ~ hierarchy + chartjunk, data = df_valid, 
